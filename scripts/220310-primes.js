async function drawHistogram() {
    ...
    const xScale = d3.scaleLinear()
        .domain([
            d3.min(dataset, d => d.start),
            d3.max(dataset, d => d.stop)
        ])
    ...
    const histBars = bounds.append('g')
        .selectAll('rect')
        .data(dataset)
        .enter()
    ...
    xAxis.append('line')
        .attr('x1', 0).attr('y1', 0)
        .attr('x2', dimensions.boundedWidth).attr('y2', 0)
    xAxis.append('text')
    ...
    yAxis.append('text').attr('class', 'small-text align-middle')
        .text('Prime count')
        .style('transform', 'rotate(-90deg)')
        .attr('x', -dimensions.boundedHeight/2)
        .attr('y', -10)
    yAxis.append('text').attr('class', 'small-text align-right')
        .text(d3.format(',')(yScale.domain()[0]))
        .attr('x', -3)
        .attr('y', dimensions.boundedHeight)
    
    const annotations = bounds.append('g')
    annotations.append('text').attr('class', 'small-text')
        .text(`${d3.format(',.2f')(maxCount/1000000)} million`)
        .attr('x', 0)
        .attr('y', yScale(maxCount)-10)
    annotations.append('text').attr('class', 'small-text align-right')
    ...
    annotations.append('line')
        ...
        .attr('x2', dimensions.boundedWidth/30-.5)
        .attr('y2', yScale(maxCount)+1)
    annotations.append('line')
        .attr('class', 'annotation-line-pop')
        .attr('x1', dimensions.boundedWidth-dimensions.boundedWidth/30 + .5)
        .attr('y1', yScale(minCount)+1)
        .attr('x2', dimensions.boundedWidth - .5)
        .attr('y2', yScale(minCount)+1)

    histBars.on('mouseenter', onMouseEnter)
    ...
    function onMouseEnter() {
        ...
        tooltip.select('.number')
            .text(`${d3.format(',')(datum.count)} primes`)
        ...
        if (leftPos >= 0) {
            tooltip.classed('bottom-pointer', true)
            ...
            tooltip.classed('bottom-pointer', false)
    ...
    function onMouseLeave() {
        ...
        d3.select(this).style('opacity', .15)
    }
}
drawHistogram()
...
async function drawHeatmap() {
    const dataset = await d3.csv('data/220310-primes/digit_count.csv', d3.autoType)
    ...
    if (width == 600) chart.classed('chart-narrow', false)
        .classed('chart-mobile', true)
    ...
    const values = dataset.map(({digit, ...placeValues}) => Object.values({...placeValues}))
    const colorScale = d3.scaleLinear()
        .domain(d3.extent(d3.merge(values)))
        .range(['#f2f2f2', '#7a7ae6'])
    digits.forEach(digit => {
        const data = dataset.filter(d => d.digit==digit)[0]
        bounds.append('g').selectAll('rect')
            .data(placeValues)
            .enter()
            .append('rect')
        ...
        bounds.append('g').selectAll('text')
            .data(placeValues)
            .enter().append('text')
    ...
    const annotationPos = dimensions.boundedWidth+dimensions.margins.right*.2

    axes.append('text').text('Number of occurences (in millions)')
    ...
    axes.append('text')
        ...
        .attr('y', yScale(5)+rectHeight*.05)
    axes.append('text')
        .attr('class', 'small-text')
        .text('cannot end with 5')
        .attr('x', annotationPos)
        .attr('y', yScale(5)+rectHeight*.5)
    axes.append('text')
        .attr('class', 'small-text')
        .text('or even digits')
        .attr('x', annotationPos)
        .attr('y', yScale(5)+rectHeight*.95)
    axes.append('line')
        .attr('class', 'annotation-line-dotted')
    ...
    digitAxis.selectAll('.digit')
        ...
        .text(d => d)
    digitAxis.selectAll('.digit-rect')
        .data(digits)
        .enter().append('rect')
        ...
        .attr('height', rectHeight)
    digitAxis.append('line')
        .attr('x1', 0).attr('x2', 0)
        .attr('y1', 0).attr('y2', dimensions.boundedHeight)
    digitAxis.append('text')
    ...
    const demoTextScale = d3.scaleBand()
        .domain(placeValues)
    ...
    legends.selectAll('.demo-digit')
        ...
        .attr('x', (d,i) => i%3==0
            ...
            : +d%3==0 ? demoTextScale(d)+digitWidth/2+7
            : demoTextScale(d)+digitWidth/2)
        .attr('y', textPos)
    legends.append('text').attr('class', 'demo-digit')
        .text('1')
    ...
    legends.selectAll('path')
        .data(placeValues)
        .enter().append('path')
        .attr('class', d => `demo-line annotation-line-dotted position-${d}`)
        .attr('d', (d,i) => {
            const topXPos = xScale(d)+rectWidth/2    
            const bottomXPos = i%3==0
                ? demoTextScale(d)+digitWidth/2-7
                : +d%3==0 ? demoTextScale(d)+digitWidth/2+7
                : demoTextScale(d)+digitWidth/2
            const topYPos = 0
            const bottomYPos = textPos-8
            const midTopYPos = (i==0||i==8) ? bottomYPos*.2
                :(i==2||i==6) ? bottomYPos*.1
                : bottomYPos*.15
            const midBottomYPos = bottomYPos*.7
            const path = [
                [topXPos, topYPos],
                [topXPos,midTopYPos],
    ...
    legends.selectAll('.demo-position')
        .data(placeValues)
        .enter().append('text').attr('class', 'small-text demo-position')
        .text(d => d)
        .attr('x', (d,i) => i%3==0
            ? demoTextScale(d)+digitWidth/2-7
            : +d%3==0 ? demoTextScale(d)+digitWidth/2+7
            : demoTextScale(d)+digitWidth/2)
        .attr('y', textPos-2) 
    legends.selectAll('.demo-position-text')
        .data(placeValues)
        .enter().append('text').attr('class', 'small-text demo-position-text')
    ...
    function rectOnMouseEnter(datum) {
        ...
        const digit = +rect.attr('class')
            .split(' ').filter(d => d.includes('digit'))[0]
            .slice(-1)
    ...
    const digitRects = d3.selectAll('.digit-rect')
    ...
    function digitOnMouseEnter() {
        ...
        bounds.append('rect').attr('class', 'hightlight-rect')
            .attr('x', 0)
            .attr('y', yScale(digit))
        ...
        tooltip.select('.digit').text(digit)
        tooltip.select('.position-container')
...
async function drawBars() {
    ...
    const bars = bounds.append('g')
            ...
            .data(dataset)
            .enter().append('rect')
            .attr('class', d => toZoom.some(string => d.pattern.includes(string)) ? `bar to-zoom` : `bar no-zoom`)
            .attr('x', 0)
    ...
    const xAxis = bounds.append('g')
    xAxis.append('text').text(0)
        .attr('class', 'tiny-text baseline-above align-middle')
        .attr('x', 0)
        .attr('y', -5)
    xAxis.append('text').text('5 million')
    ...
    milMarks.selectAll('text')
        .data(d3.range(5))
        .enter()
        .append('text')
        .attr('class', 'million-marks')
        .attr('x', d => dimensions.boundedWidth-5 + d*rectHeight*.5)
    ...
    const counts = bounds.append('g')
        ...
        .data(dataset)
        .enter().append('text')
        .attr('class', d => toZoom.some(string => d.pattern.includes(string)) ? `tiny-text to-zoom` : `tiny-text no-zoom`)
        .attr('x', d => d.pattern=='Pair' ? dimensions.boundedWidth+milMarksWidth-2 : xScale(d.count)+3)
        .attr('y', d => yScale(d.pattern)+rectHeight/2)
        .text(d => d3.format(',')(d.count))

    const examples = bounds.append('g')
        .selectAll('text')
        .data(dataset)
        .enter().append('text')
    ...
    const annotation = chart.append('text')
        ...
        .text('Patterns can be found in the middle of a long numbers, hence primes (which they themselves cannot be even) can include plenty of even patterns like 222,  888, or 9696.')
    ...
    legends.append('line')
        .attr('class', 'annotation-line-dotted')
        .attr('x1', 0).attr('x2', annotationWidth)
        .style('transform', `translateY(${dimensions.boundedHeight-annotationHeight-10}px)`)
    
    const zoomRect = bounds.append('rect')
        .attr('class', 'zoom-rect')
        .attr('x', 0).attr('y', yScale('Six of')-3)
        .attr('width', 45).attr('height', yScale('Eight of')-yScale('Six of')+rectHeight+6)
    const zoomText = bounds.append('text')
        .attr('class', 'zoom-rect-text tiny-text')
        .text('hover to zoom')
        .attr('x', 50)
        .attr('y', yScale('Seven of')+rectHeight/2)
    zoomRect.on('mouseenter', zoomOnMouseEnter)
    ...
    function zoomOnMouseEnter() {
        ...
        const zoomXScale = d3.scaleLinear()
            .domain([0, 2000])
            .range([0, zoomWidth])

        zoomRect.attr('width', zoomWidth)
        barsToZoom.attr('width', d => zoomXScale(d.count))
            .style('opacity', .3)
        barsOther.style('fill', 'silver')
        textsToZoom.transition(getTransition())
            .attr('x', d => zoomXScale(d.count)+5)
        textsOther.style('opacity', .15)
        milMarks.style('opacity', .15)
        legends.style('opacity', .15)
        annotation.style('opacity', .15)
        xMax.text('2,000')
    }
    function zoomOnMouseLeave() {
        zoomText.style('opacity', 1)
        zoomRect.attr('width', 45)
        barsToZoom.attr('width', d => 2)
            .style('opacity', .15)
...
async function showPrimes() {
    ...
    const filter = d3.select('#filter-input')
        .on('change', patternInputOnChange)
    ...
    function countPattern(pattern) {
        ...
        d3.range(binsCount).forEach(d => {
            let tempObj = {}
            tempObj.pageIndex = d
            tempObj.hasPattern = data[d].filter(p => p.prime.includes(pattern)).length
    ...
    const pagesWrapper = d3.select('.pages')
        .on('mouseleave', pagesOnMouseLeave)
    const pages = pagesWrapper.selectAll('.page')
        .data(patternCount)
        .join('div')
        .attr('class', 'page')
    ...
    function pageClick(event, datum) {
        d3.selectAll('.page').filter(d => d!=datum)
            .classed('page-selected', false)
            .style('color', d => colorScale(d.hasPattern))
        d3.select(this).classed('page-selected', true)
            .style('color', '#f2f2f2')
        ...
        selected.forEach(d => {
            ...
            .style('opacity', d.prime.includes(pattern) ? 1 : .2)
    ...
    function patternInputOnChange() {
        ...
        if (max!=0) {
            ...
            pages.data(patternCount)
            .join('div')
            .style('color', d => colorScale(d.hasPattern))
            .style('pointer-events', 'all')
            filter.style('color', '#222222')
                .style('border-bottom-color', '#0000d8')
            alert.style('display', 'none')
        } else {
            pages.style('color', 'gainsboro')
                .style('pointer-events', 'none')
            filter.style('color', '#FE4A49')
                .style('border-bottom-color', '#FE4A49')
